...
ciclo_generacion._guardar_cdr = lambda ruc, tip, num, cdr, msg: CDRS.append(num)

ESTADO = {"valor": ("no_registrado", None, "no lo tiene")}
ciclo_generacion.estado_en_sunat = lambda ruc, tip, num: ESTADO["valor"]
...
def reset():
    ...
    if os.path.exists(reintentos._REINTENTOS_PATH): os.remove(reintentos._REINTENTOS_PATH)

def sin_espera(num):
    with open(reintentos._REINTENTOS_PATH) as f: d = json.load(f)
    d[num]["esperar_hasta"] = 0
    with open(reintentos._REINTENTOS_PATH, "w") as f: json.dump(d, f)
...
for vuelta in range(6):
    ...
    if os.path.exists(reintentos._REINTENTOS_PATH): sin_espera("F003-1")
check(MARCADOS.count("F003-1") == 6, f"reintento 6 veces, mas que el tope de 3 ({MARCADOS.count('F003-1')})")
reg = json.load(open(reintentos._REINTENTOS_PATH))["F003-1"]
...
ciclo_generacion.resetear_rechazados(None, RUC)
guardado = json.load(open(reintentos._REINTENTOS_PATH))["F003-5"]
...
ciclo_generacion.resetear_rechazados(None, RUC)
check("F003-7" in json.load(open(reintentos._REINTENTOS_PATH)), "quedo registrado")
reintentos._limpiar_reintento("F003-7")
check("F003-7" not in json.load(open(reintentos._REINTENTOS_PATH)), "al aceptarse se limpia todo (contador y espera)")
