...
(ver dominio.cdr._reconciliar_numeracion), que el cierre de la bandeja sea
idempotente, y que un fallo al cerrar en la BD deje el error visible en vez de
borrarlo.
"""
...
import aplicacion.ciclo_cdr as cc
import sfs.bd as sfs_bd
from dominio.cdr import _TIPO_RC, _extraer_numeracion, _reconciliar_numeracion
logging.disable(logging.CRITICAL)

TMP = tempfile.mkdtemp()
cc.SFS_RPTA_DIR  = os.path.join(TMP, "RPTA");      os.makedirs(cc.SFS_RPTA_DIR)
cc.DIR_PROCESADOS = os.path.join(TMP, "procesados"); os.makedirs(cc.DIR_PROCESADOS)
cc.DIR_ERRORES    = os.path.join(TMP, "errores");    os.makedirs(cc.DIR_ERRORES)
RUC = "20605858601"
...
def poner_cdr(nombre, ident):
    ruta = os.path.join(cc.SFS_RPTA_DIR, nombre)
...
CERRADOS = []
cc.conectar_bd = lambda: None
cc._actualizar_sql_cdr = lambda conn, num, parsed: (CERRADOS.append(num), True)[1] if num in EXISTEN else False
...
check(os.listdir(cc.DIR_PROCESADOS) and not os.listdir(cc.DIR_ERRORES), "el zip fue a procesados/")

# ---- criterio 2: el CDR normal sigue igual ----
EXISTEN = {"F003-009595"}
CERRADOS.clear()
for d in (cc.DIR_PROCESADOS, cc.DIR_ERRORES):
...
check(os.listdir(cc.DIR_ERRORES) and not os.listdir(cc.DIR_PROCESADOS), "criterio 5: el zip fue a errores/")
...
nombre_rc = f"R{RUC}-{_TIPO_RC}-RC-20260903-001.zip"
check(_extraer_numeracion(nombre_rc) == "RC-20260903-001",
      "criterio 3: la numeracion del resumen sale del nombre")
check(_reconciliar_numeracion("RC-20260903-001", nombre_rc) == "RC-20260903-001",
      "criterio 3: y la reconciliacion no la toca")

# ---- series distintas: el nombre nunca pisa una numeracion que no le corresponde ----
check(_reconciliar_numeracion("B001-000009", f"R{RUC}-01-F003-009571.zip") == "B001-000009",
