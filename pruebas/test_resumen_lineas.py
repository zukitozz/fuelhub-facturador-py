...
import aplicacion.lecturas as lecturas
import estado.resumenes as resumenes
import estado.reintentos as reintentos
logging.disable(logging.CRITICAL)

TMP = tempfile.mkdtemp()
resumenes._RESUMENES_PATH = os.path.join(TMP, "resumenes.json")
reintentos._REINTENTOS_PATH = os.path.join(TMP, "reintentos.json")
ciclo_cdr.MAX_REINTENTOS_RECHAZO = 3
...
def reset():
    ...
    for p in (resumenes._RESUMENES_PATH, reintentos._REINTENTOS_PATH):
...
resumenes._registrar_resumen("RC-20260826-001", boletas)
parsed = {"status": "ACEPTADO", "lineas": []}
ok = ciclo_cdr._actualizar_sql_cdr(None, "RC-20260826-001", parsed)
...
reset()
boletas = [f"B003-{i:06d}" for i in range(1, 201)]
resumenes._registrar_resumen("RC-20260826-002", boletas)
parsed = {"status": "ACEPTADO", "lineas": [("B003-000150", "3103", "Boleta ya declarada")]}
ok = ciclo_cdr._actualizar_sql_cdr(None, "RC-20260826-002", parsed)
...
check(reintentos._reintentos_de("B003-000150") == 1, "queda contado como 1er intento")
...
for intento in range(1, 4):
    resumenes._registrar_resumen(f"RC-20260826-00{intento+2}", boletas)
    parsed = {"status": "ACEPTADO", "lineas": [("B003-000150", "3103", "Boleta ya declarada")]}
    ciclo_cdr._actualizar_sql_cdr(None, f"RC-20260826-00{intento+2}", parsed)
print(f"  -> tras 3 intentos, _reintentos_de = {reintentos._reintentos_de('B003-000150')}")
check(reintentos._reintentos_de("B003-000150") >= ciclo_cdr.MAX_REINTENTOS_RECHAZO, "agoto los 3 reintentos")
...
lecturas._bd = lambda: FakeBDPendientes()
candidatas = lecturas.obtener_boletas_para_resumen(None)
...
resumenes._registrar_resumen("RC-20260826-010", boletas)
parsed = {"status": "ACEPTADO", "lineas": [(b, "3103", "obs") for b in boletas]}
ok = ciclo_cdr._actualizar_sql_cdr(None, "RC-20260826-010", parsed)
